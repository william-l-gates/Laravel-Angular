...
class OrderController extends BaseController {	
	...
	public function orderList(){
		...
		$order_list = DB::select(DB::raw("SELECT ORE.Requested_Ship_Date,ORE.id as ORDER_ID,job.id AS Job_id,job.Channel_id,
			JTS.id AS JobTypeStatus_Id,JTS.status_name AS Status_Name,ORE.Order_Date,ORE.ChannelName,ORE.ShipName,
			...
			ORE.CarrierServiceName,ORE.Comments,OREI.Order_Qty,OREI.Active,ORE.created_at AS Create_ts,ORE.updated_at AS Modify_ts,
			...
			WHERE ORE.Active = ".'"1"'." AND OREI.Active = ".'"1"'." GROUP BY ORE.id
	...
	$order_list = DB::select(DB::raw("SELECT ORE.ORDER_ID, ORE.Order_Date,ORE.ChannelName,
		...
		LEFT JOIN orderitems AS OREI ON OREI.Order_Id=ORE.Order_Id LEFT JOIN products AS PRO ON PRO.id = OREI.Order_Item
		LEFT JOIN jobs AS JOB ON JOB.id = ORE.Job_Id where OREI.Balance_Qty != ".'"0"'." Group By ORE.ORDER_ID"));
	return Response::json(array('response' => $order_list));
	}
	...
	$channel_wise_warehouse_product = DB::select(DB::raw("SELECT pro.id,CONCAT(pro.RefItemId,'-',pro.Product_Name,'-',pro.SKU) as Product_Name,
pro.Channel_Id,cha.channel_name FROM PRODUCTS AS pro
		LEFT JOIN channels AS cha ON cha.id = pro.Channel_Id
		WHERE PRO.Active=".'"1"'." AND cha.Active=".'"1"'." AND cha.id=". Input::json('channel_id')));
	return Response::json(array('response' => $channel_wise_warehouse_product));	
	}	
	
	// Get Stock Count	
	public function getAvilableQtyDetails(){
	$avilable_qty = DB::select(DB::raw("SELECT CH.channel_name as Channel_Name,PRO.id as Product_Id, INVENT.Stock_Count,
		WARE.warehouse_name as Warehouse_Name,PRO.Product_Name from InventoryItems AS INVENT
		LEFT JOIN Channels AS CH ON CH.id = INVENT.Channel_Id
		LEFT JOIN Warehouses AS WARE ON WARE.id = INVENT.Warehouse_Id
		LEFT JOIN Products AS PRO ON PRO.id = INVENT.Product_Id
	...
	$channel_wise_warehouse = DB::select(DB::raw("SELECT INVENT.Warehouse_Id, WARE.Warehouse_Name,
			INVENT.Channel_Id,CHA.channel_name as Channel_Name from inventoryitems AS INVENT
	...
	$order_list = DB::select(DB::raw("select ore.ORDER_ID,CHE.Channel_Name,job.Channel_Id,job.id,
		...
		LEFT JOIN channels AS CHE ON CHE.id=JOB.Channel_Id
	...
	$new_order_list = DB::select(DB::raw("SELECT ORE.Requested_Ship_Date,ORE.id,job.Job_Id,
	...
	**/	
	public function getChannelName($id){
		$channel_id = $id; 
		$channel__name_dat = DB::select(DB::raw("SELECT channel_name FROM channels WHERE id =".$channel_id));
	...
	**/
	...
	public function getEditOrderDetails(){
		$order_list = DB::select(DB::raw("SELECT ORE.Requested_Ship_Date,ORE.id as ORDER_ID,
			ORE.CarrierName,ORE.CarrierServiceName,OREI.id as Order_Item_Id, OREI.Active, ORE.Job_Id,
			...
			ORE.Comments,ORE.ShipmentsTotalCost,ORE.created_at as Create_ts,ORE.updated_at as MODIFY_TS,
			...
			job.Job_Status_Id, JTS.status_name as Status_Name,
			CHE.channel_name as Channel_Name FROM orders AS ORE
			LEFT JOIN orderitems AS OREI ON OREI.Order_Id=ORE.id
			LEFT JOIN Jobs AS job ON job.id=ORE.Job_Id
			LEFT JOIN job_type_status AS JTS ON JTS.id=JOB.Job_Status_Id
			LEFT JOIN products AS PRO ON PRO.id=OREI.Order_Item
			LEFT JOIN channels AS CHE ON CHE.id=JOB.Channel_Id
		WHERE OREI.Active = ".'"1"'." AND ORE.id = ". Input::json('id')));
			
		$tot_shipment_list = DB::select(DB::raw("SELECT ore.id as Order_Id,ore.Order_Date,jstatus.Status_Name,
			ore.id as Order_Id,ship.id as Shipment_Id, job.Job_Type_Id, job.Job_Status_Id,SUM(ShipmentCost) AS shipmentTotalCost FROM `Shipments` as ship
				...
				WHERE job. Job_Type_Id=".'"4"'." AND job.Job_Status_Id=".'"4"'." AND job.Active=".'"1"'." AND ore.id=".Input::json('id')));
		
		$channel_list = DB::table('channels')->get();
		$product_list = DB::table('products')->get(); 
		$carrier_list = DB::table('Carrier')->get();
		$carrier_service_list = DB::table('CarrierServices')->get();
		$order_product = DB::select(DB::raw("SELECT pro.id as Product_Id,pro.Product_Name FROM Products as pro
			LEFT JOIN InventoryItems AS it ON it.Product_Id = pro.id
			LEFT JOIN Jobs AS j ON j.Channel_Id = it.Channel_Id 
			LEFT JOIN Orders AS ord ON ord.Job_Id = j.id
			WHERE ord.id=".Input::json('id')));
	...
	public function editOrderDetails(){						
		...
		$update_job_details = DB::select(DB::raw('update jobs set `Channel_Id` ='. Input::json('channel_name'). ',`Job_Status_Id` = '. Input::json('status_id'). '  where id = ' . Input::json('jobid')));
		$update_order_details = DB::select(DB::raw('update orders set `Order_Date` = ' . "'". Input::json('orderdate'). "'".',
			...
			`updated_at` =  "'.$MODIFY_TS.'",
			`Comments` ='. "'" . Input::json('comments')."'" .' 
		WHERE id = ' . Input::json('orderid')));
				...
				if (!(in_array($i, $deleted_item_array))) {		
					if(isset(Input::json('order_item')['order_item'.$i])){				
						if(isset(Input::json('order_item_id')['order_item_id'.$i])){					
							...
							WHERE id = ' . Input::json('order_item_id')['order_item_id'.$i]));
						}else{	
							$prod_list = $this->getProductDetailsByProductId(Input::json('order_item')['order_item'.$i]);
							if(count($prod_list)>0){
								$order_item_dtl = new Orderitems;				
	...
	*/	
	public function deleteOrderDetails(){
		$delete_order_details = DB::select(DB::raw('UPDATE orders SET `ACTIVE` = "0" WHERE id = ' . Input::json('id')));
	...
	public function getOrderListCount(){
		$requested_ch_right ="AllowOrderView";
		$usergroup = User::find(Auth::id())->usergroup()->first();
		$order_list = DB::select(DB::raw("SELECT ORE.Requested_Ship_Date,ORE.id,job.id AS Job_Id,job.Channel_id,
			JTS.id AS JobTypeStatus_Id,JTS.status_name AS Status_Name,ORE.Order_Date,ORE.ChannelName,ORE.ShipName,
			ORE.ShipAddress1,ORE.ShipAddress2,ORE.ShipCompany,ORE.ShipCity,ORE.ShipZip,ORE.ShipState,
			ORE.ShipCountry,ORE.ShipEmail,ORE.BillName,ORE.BillAddress1,ORE.BillAddress2,
			ORE.BillCompany,ORE.BillCity,ORE.BillZip,ORE.BillState,ORE.BillCountry,ORE.BillEmail,
			...
			HAVING job.Channel_id in(select ugchr.channel_id
										from user_group_channel_right ugchr, channel_right chr
										  where ugchr.userGroupID = ".$usergroup->id."
				 								and chr.channel_right_name = '".$requested_ch_right."')"));
		$new_order_list = DB::select(DB::raw("SELECT ORE.id as ORDER_ID,ORE.Order_Date,ORE.Job_Id,JOB.Job_Status_Id,OREI.Order_Qty,OREI.Order_Item,OREI.Active,PRO.Product_Name,pro.Price FROM `orders` AS ORE LEFT JOIN orderitems AS OREI ON OREI.Order_Id=ORE.id LEFT JOIN products AS PRO ON PRO.id = OREI.Order_Item LEFT JOIN jobs AS JOB ON JOB.id = ORE.Job_Id where ORE.Active = ".'"1"'." and JOB.Job_Status_Id = ".'"1"'." and OREI.Active = ".'"1"'." group by ORE.id"));

		$inprogress_order_list = DB::select(DB::raw("SELECT ORE.id as ORDER_ID,ORE.Order_Date,ORE.Job_Id,JOB.Job_Status_Id,OREI.Order_Qty,OREI.Order_Item,OREI.Active,PRO.Product_Name,pro.Price FROM `orders` AS ORE LEFT JOIN orderitems AS OREI ON OREI.Order_Id=ORE.id LEFT JOIN products AS PRO ON PRO.id = OREI.Order_Item LEFT JOIN jobs AS JOB ON JOB.id = ORE.Job_Id where ORE.Active = ".'"1"'." and JOB.Job_Status_Id = ".'"2"'." and OREI.Active = ".'"1"'." group by ORE.id"));
		$closed_order_list = DB::select(DB::raw("SELECT ORE.id as ORDER_ID,ORE.Order_Date,ORE.Job_Id,JOB.Job_Status_Id,OREI.Order_Qty,OREI.Order_Item,OREI.Active,PRO.Product_Name,pro.Price FROM `orders` AS ORE LEFT JOIN orderitems AS OREI ON OREI.Order_Id=ORE.id LEFT JOIN products AS PRO ON PRO.id = OREI.Order_Item LEFT JOIN jobs AS JOB ON JOB.id = ORE.Job_Id where ORE.Active = ".'"1"'." and JOB.Job_Status_Id = ".'"3"'." and OREI.Active = ".'"1"'." group by ORE.id"));
        $listChannel = DB::select(DB::raw("SELECT jobs.channel_id,channels.*
                                            FROM orders
                                            LEFT JOIN jobs
                                            ON jobs.id = orders.Job_id
                                            LEFT JOIN channels
        ...
        if(count($listChannel)>0){
            for($i=0;$i<count($listChannel); $i++){
                ...
                $listChannelStatus =DB::select(DB::raw("SELECT job_type_status.id,job_type_status.status_name
                                                ...
                                                WHERE  job_type_name='Order' AND  channel_id =".$arrayList[$i]['id']));
                    for($j=0; $j<count($listChannelStatus); $j++){
                    $statusID = $listChannelStatus[$j]->id;
                        $channelID = $arrayList[$i]['id'];
                        $listOrderList = DB::select(DB::raw("SELECT ORE.Requested_Ship_Date,ORE.id,job.id AS Job_Id,job.Channel_id,
                                                                    JTS.id AS JobTypeStatus_Id,JTS.status_name AS Status_Name,ORE.Order_Date,ORE.ChannelName,ORE.ShipName,
                                                                    ORE.ShipAddress1,ORE.ShipAddress2,ORE.ShipCompany,ORE.ShipCity,ORE.ShipZip,ORE.ShipState,
                                                                    ORE.ShipCountry,ORE.ShipEmail,ORE.BillName,ORE.BillAddress1,ORE.BillAddress2,
                                                                    ORE.BillCompany,ORE.BillCity,ORE.BillZip,ORE.BillState,ORE.BillCountry,ORE.BillEmail,
                                                                    ...
                                                                    LEFT JOIN job_type_status AS JTS ON JTS.id=JOB.Job_Status_Id
                                                                    LEFT JOIN products AS PRO ON PRO.id = OREI.Order_Item
                                                                    WHERE ORE.Active = ".'"1"'." AND OREI.Active = ".'"1"'." and JTS.id=".$statusID." GROUP BY ORE.id
                                                                    HAVING job.Channel_id in(select ugchr.channel_id
                                                                                                from user_group_channel_right ugchr, channel_right chr
                                                                                                  where ugchr.userGroupID = ".$usergroup->id."
                                    and chr.channel_right_name = '".$requested_ch_right."')"));
		...
		return Response::json(array('all_order_list' =>$order_list, 'new_order_count' => $new_order_list, 'inprogress_order_count' => $inprogress_order_list, 'closed_order_count' => $closed_order_list,'all_order_channel_list' =>$arrayList));
	}
	...
	*/
    ...
    public function channel($id){
        ...
        $order_list = DB::select(DB::raw("SELECT ORE.Requested_Ship_Date,ORE.id as ORDER_ID,job.id AS Job_id,job.Channel_id,
			...
			LEFT JOIN job_type_status AS JTS ON JTS.id=JOB.Job_Status_Id
			LEFT JOIN products AS PRO ON PRO.id = OREI.Order_Item
			WHERE ORE.Active = ".'"1"'." AND OREI.Active = ".'"1"'." and job.Channel_id = ".$id." GROUP BY ORE.id
			HAVING job.Channel_id in(select ugchr.Channel_id
										from user_group_channel_right ugchr, channel_right chr
										  where ugchr.userGroupID = ".$usergroup->id."
				 								and chr.channel_right_name = '".$requested_ch_right."')"));
    ...
    public function status($id){
        ...
        $order_list = DB::select(DB::raw("SELECT ORE.Requested_Ship_Date,ORE.id as ORDER_ID,job.id AS Job_id,job.Channel_id,
			...
			LEFT JOIN job_type_status AS JTS ON JTS.id=JOB.Job_Status_Id
			LEFT JOIN products AS PRO ON PRO.id = OREI.Order_Item
			WHERE ORE.Active = ".'"1"'." AND OREI.Active = ".'"1"'." and job.Job_Status_Id = ".$id." GROUP BY ORE.id
			HAVING job.Channel_id in(select ugchr.Channel_id
										from user_group_channel_right ugchr, channel_right chr
										  where ugchr.userGroupID = ".$usergroup->id."
				 								and chr.channel_right_name = '".$requested_ch_right."')"));
